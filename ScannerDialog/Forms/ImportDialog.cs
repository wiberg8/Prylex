...
namespace ScannerDialog
{
    public partial class ImportDialog : Form
    {
        ...
        private string _selectedImportFil;
        private string SelectedImportFil
        {
            get{ return _selectedImportFil; }
            set{ _selectedImportFil = value; txtImportFil.Text = _selectedImportFil; }
        }
        ...
        private void cmdTestImport_Click(object sender, EventArgs e)
        {
            string persNrRegex = "^[0-9]{8}[-][0-9]{4}$";
            char splitChar = ';';

            if (!File.Exists(SelectedImportFil))
            {
                MessageBox.Show("Välj import fil");
                return;
            }
            string[] fileData = File.ReadAllLines(SelectedImportFil);
            foreach(string line in fileData)
            {
                string[] lineSplit = line.Split(splitChar);
                if (lineSplit.Length >= 3 && !string.IsNullOrWhiteSpace(cbTillhorighet.Text))
                {
                    ...
                    if (Regex.IsMatch(person.PersNr, persNrRegex))
                    {
